#!/usr/bin/env python3
#adding clean data after every MTL-NN step
import os
import argparse
import random
import ntpath
...
def read_pos_tree_file(data):
    ...
    file2= open('/data/users/dpaul/Thesis/Practical/mtl_chunk_pos/50_noise/Full.pos.data.train','w')
    n=0
    for line in content:   
       n=n+1
       if n <=904383:
    ...
    file2.close()
    file2= open('/data/users/dpaul/Thesis/Practical/mtl_chunk_pos/50_noise/Full.pos.data.train','a')
    n=0
    file1= open(data,'r')
    for line in lines:   
       n=n+1
